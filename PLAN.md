...
- **История чата** — просмотрщик реализован (`GET /chat/channels`, `GET /chat?channelId=&before=&limit=`,
  ...
  2026-09-23: `channelId` — повторяемый параметр (≤100, в Mongo — `$in`), фильтр ленты на экране
  сделан выпадающим мультивыбором (только историчные каналы). Там же панель «Отправить от бота»
  (`POST /bot/channel/{id}/message`, текст ≤2000; с вложениями — multipart, ≤10 файлов по 25 МБ,
  суммарно ≤50 МБ) в живые текстовые каналы из `/picker`; при N>1 каналов отправка идёт
  последовательно с паузой 550 мс под бакет 5 burst / 2/с и с двойным подтверждением.
